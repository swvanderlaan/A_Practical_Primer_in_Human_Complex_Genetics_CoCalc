...
```{r load-sample-callrate-book, eval=FALSE, echo = FALSE, include = FALSE}
# to test the book locally
raw_IMISS <- data.table::fread("data/dummy_project/rawdata.imiss")
...
```{r showsamplecallrate, fig.align='center', fig.cap='Per sample call rate.', fig.show = TRUE, eval=TRUE, echo=FALSE}
...
```{r load-heterozygosity, eval = FALSE, warning = FALSE}
raw_HET <- data.table::fread("dummy_project/rawdata.clean.ultraclean.het")
```

```{r load-heterozygosity-book, eval = FALSE, echo = FALSE, include = FALSE}
# to test the book locally
raw_HET <- data.table::fread("data/dummy_project/rawdata.clean.ultraclean.het")
```

```{r load-heterozygosity-calc, eval = FALSE, warning = FALSE}
...
```{r showheterozygosity, fig.align='center', fig.cap='Heterozygosity as a function of SNP call rate.', fig.show = TRUE, eval=TRUE, echo=FALSE}
...
```{r create-fail-imisshet-qc, eval = FALSE, warning = FALSE}
...
```{r view-fail-imisshet-qc, eval = FALSE}
View(raw_IMISSHETsub)
```

If all is right, you'd have something like Table \@ref(tab:failedcallratehet).

```{r failed-callrate-het-book, eval = TRUE, echo = FALSE}
raw_IMISSHETsub <- data.table::fread("data/dummy_project/fail-raw_IMISSHETsub.txt")
```

```{r tab.id = "failedcallratehet", tab.cap = "Failed samples due to sample call rates and heterozygosity rate.", eval = TRUE, echo=FALSE}
library("flextable")
ft <- flextable(
  raw_IMISSHETsub,
  # col_keys = names(data),
...
```{r, tab.id='showrelatedness', tab.cap='Familial relations and % DNA shared.', eval = TRUE, echo=FALSE}
library("flextable")
ft <- flextable(
  relatedness,
  # col_keys = names(data),
...
```{r tab.id='showibdcallissues', tab.cap='Failed IBD and callrate.', eval = FALSE}
View(ibdcallissues)
```

```{r ibdcallissues-book, eval = TRUE, echo = FALSE}
ibdcallissues <- data.table::fread("data/dummy_project/fail-IBD-QC.txt")
names(ibdcallissues)[names(ibdcallissues) == "V1"] <- "FID"
names(ibdcallissues)[names(ibdcallissues) == "V2"] <- "IID"
```

```{r tab.id='showibdcallissuesbook', tab.cap='Failed IBD and callrate.', eval = TRUE, echo=FALSE}
library("flextable")
ft <- flextable(
  ibdcallissues,
  # col_keys = names(data),
...
```{r load-pca-1kg-part1-book, eval = FALSE, echo = FALSE, include = FALSE}
# to test the book locally
PCA_eigenval <- data.table::fread("data/dummy_project/rawdata.1kg_phase1.clean.eigenval")
PCA_eigenvec <- data.table::fread("data/dummy_project/rawdata.1kg_phase1.clean.eigenvec")
ref_pop_raw <- data.table::fread("data/ref_1kg_phase1_all/1kg_phase1_all.pheno")
dummy_pop <- data.table::fread("data/dummy_project/rawdata.fam")
```

```{r manage-pca-1kg-part1, eval = FALSE, warning = FALSE}
# Rename some 
...
```{r load-pca-1kg-part3, eval = FALSE}
...
```{r showpca1kg, fig.align='center', fig.cap='PCA - Your data vs. 1000g.', fig.show = TRUE, eval=TRUE, echo=FALSE}
