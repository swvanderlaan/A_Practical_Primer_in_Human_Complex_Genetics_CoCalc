...
```{r callrate, message=FALSE, warning=FALSE, out.width='80%', fig.align='center', fig.asp=.75, fig.cap='Call rate'}
...
The grey dashed line in Figure \@ref(fig:callrate) indicates the mean call rate, while the red dashed line indicates the threshold we had determined above.
...
Often there is a correlation between heterozygosity rate and missing data. Thus, we should plot the observed heterozygosity rate per individual on the x-axis and the proportion of missing SNP, that is the 'SNP call rate', per individuals on the y-axis (Figure \@ref(fig:heterozygosity)).
...
```{r heterozygosity, message=FALSE, warning=FALSE, out.width='80%', fig.align='center', fig.asp=.75, fig.cap='Heterozygosity rate as a function of missing genotypes'}
...
We calculate Identity-by-Descent (IBS), to identify duplicated and related samples (Table \@ref(tab:tab-relatedness)). IBS is measured by calculating pi-hat, which is in essence the proportion of the DNA that a pair of samples share. To calculate this, we needed this ultraclean dataset, without low-quality SNPs and without high-LD regions. Now we are ready to calculate the IBS.
...
```{r tab-relatedness, tidy = FALSE, eval = TRUE, echo = FALSE}
knitr::kable(
  relatedness, 
...
We can now identify all pairs of individuals with an IBD > 0.185. The code looks at the individual call rates stored in rawdata.imiss and outputs the IDs of the individual with the lowest call rate to ‘fail-IBD-QC.txt’ for subsequent removal (Table \@ref(tab:ibdcallissues)).
...
```{r ibdcallissues, tidy = FALSE, eval = TRUE, echo = FALSE}
knitr::kable(
  ibdcallissues, 
...
Derive PC1 and PC2 thresholds so that only individuals who match the given ancestral population are included. For populations of European descent, this will be either the CEU or TSI HapMap3 individuals (Figure \@ref(fig:pca-hapmap3)). Here, we chose to exclude all individuals with a second principal component score less than 0.072. 
...
You could do the above again but now with projecting the 1000G phase 1 populations. The all the 1000G phase 1 data is provided as tutorial data (), as well as a subset including _only_ the variants in our `rawdata`. You can try and run the PCA with 1000G and project the results -- let's do that in our spare time and continue for now with the QC based on HM3. But please, do show us the results tomorrow ... :-) For your convenience there are some codes below which you may need.
